...
// Der aktuelle Verzeichnispfad, in dem das Programm läuft
...
fs.readdir(currentDirectory, (err, files) => {
  ...
  // Alle Dateien im aktuellen Verzeichnis ausfiltern
  const onlyFiles = files.filter((file) => {
    const fullPath = path.join(currentDirectory, file);
    return fs.statSync(fullPath).isFile(); // Nur Dateien behalten
  });
  ...
  for (const file of onlyFiles) {
    const content = fs.readFileSync('E:\\Programmieren\\WebStorm\\terraforming-mars\\src\\server\\cards\\base\\' + file);

    const nameRegex = /CardName\.\w+/;
    const match2 = nameRegex.exec(content.toString());
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    const cardName = (match2?.[0].split('.')[1])!;
    // console.log(CN[cardName as keyof typeof CN]);
    ...
    if (match) {
      ...
      if (res.includes('BUILDING') && res.includes('SPACE')) {
        space.push(CN[cardName as keyof typeof CN]);
        building.push(CN[cardName as keyof typeof CN]);
      } else if (res.includes('SPACE')) {
        space.push(CN[cardName as keyof typeof CN]);
      } else if (res.includes('BUILDING')) {
        building.push(CN[cardName as keyof typeof CN]);
      }
    }
    if (content.includes('MiningCard')) building.push(CN[cardName as keyof typeof CN]);
  }
  console.log(building.join(','));
