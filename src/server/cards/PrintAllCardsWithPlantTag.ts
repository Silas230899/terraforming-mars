...
fs.readdir(path.join(currentDirectory, directory), (err, files) => {
  if (err) {
  ...
  const onlyFiles = files.filter((file) => {
    const fullPath = path.join(currentDirectory, directory, file);
  ...
  for (const file of onlyFiles) {
    const content = fs.readFileSync(path.join(currentDirectory, directory, file));
    ...
    if (match) {
      ...
      if (res.includes('PLANT')) {
        plants.push(CN[cardName as keyof typeof CN]);
      }
    }
  }
